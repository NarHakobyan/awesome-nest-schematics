...
import { ServiceOptions } from './command.schema';
...
function transform(options: ServiceOptions): ServiceOptions {
  ...
  options.path = normalizeToKebabOrSnakeCase(join(`/modules/${options.moduleName}` as Path, location.path, '/commands/' as Path));
